...
export class HttpResourceService<T = any> {
    ...
    show<R = T>(endpoint: string, id: string | number): Observable<R> {
        ...
        return this.http.get<R>(`${this.getBaseUrl()}/${endpoint}/${id}`, this.buildHeaders()).pipe(
            tap(data => {
                if (this.executionMode === 'server') {
    ...
    delete(endpoint: string, id: string | number): Observable<any> {
        return this.http.delete(`${this.getBaseUrl()}/${endpoint}/${id}`, this.buildHeaders());
    }

    post<R = T>(endpoint: string, payload: any): Observable<R> {
        return this.http.post<R>(`${this.getBaseUrl()}/${endpoint}`, payload, this.buildHeaders());
    }

    put<R = T>(endpoint: string, id: string | number, payload: any): Observable<R> {
        return this.http.put<R>(`${this.getBaseUrl()}/${endpoint}/${id}`, payload, this.buildHeaders());
    }
