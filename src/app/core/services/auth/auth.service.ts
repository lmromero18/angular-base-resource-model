...
export class AuthService {
  ...
  private getStorageItem(key: string): string | null {
    if (this.isBrowser) {
      ...
      if (this.request.headers.cookie) {
        ...
        return found
          ? decodeURIComponent(found.substring(key.length + 1))
          : null;
      }
    }
    return null;
  ...
  public logout(
    urlAuth?: string,
    routeDefault: string = '/Ingresar',
  ): Observable<void> {
    return new Observable<void>((observer) => {
      const token = this.getToken();
      if (token) {
        this.http
          .get(`${urlAuth ?? environment.urlAuth}/api/v3/auth/logout`)
          .subscribe(
            () => {
              this.clearPayload();
  ...
  public getUser(): any {
    try {
      ...
      this.user = tok ? this.jwtHelperService.decodeToken(tok) : undefined;
  ...
  public hasPermissions(permissionsArr: string[]): boolean {
    return _.some(
      ...
      (r) =>
        r.all_permissions ||
        _.some(r.permissions, (p: any) =>
          permissionsArr.includes(p.cod_permission),
        ),
    );
  }
      ...
      .map((r) =>
        _(r?.permissions)
          .filter(
            (p: any) =>
              p?.module === module ||
              (Array.isArray(module) && module.includes(p?.module)),
          )
          .map('cod_permission')
          .value(),
      )
  ...
  public isPermissionAssigned(permission: string): boolean {
    ...
    return (
      !permission ||
      (!!this.getToken() &&
        (permissions.includes(permission) || this.isSuperUser()))
    );
  }
