...
import { SERVER_PUBLIC_JWK_KEY } from '../core.states.key';
...
export class AppCryptoService {
  ...
  public get isReady() {
    return this.ready && !!this.serverPublicKey;
  }
  ...
  public async encryptPayload(
    plain: any,
  ): Promise<{ envelope: Envelope; nonce: string }> {
    if (!this.isReady || !this.serverPublicKey)
